...
//        val minute = 15;
        ...
        val clockWordText = prefix + GetWordFromNumber(hour).lowercase() + postfix;
        textPaint.color = Color.WHITE;
        textPaint.textSize = 45.0f;
        val textBounds = Rect();
        textPaint.getTextBounds(clockWordText, 0, clockWordText.length, bounds);
        val textWidth = textPaint.measureText(clockWordText);

        canvas.drawText(
            clockWordText,
            canvas.width / 2.0f - textWidth / 2,
            canvas.height / 2.0f + textBounds.top / 2,
            textPaint
        );
